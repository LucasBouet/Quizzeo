    ...
    <div class="max-w-4xl mx-auto p-6 bg-white/70 backdrop-blur-md rounded-2xl shadow-2xl border border-white/50">
        ...
        <?php if ($creatorRole == 'Entreprise'): ?>
            <form method="post" action="#" class="card bg-gray-300 shadow-xl p-8 space-y-6 text-stone-800">
                <h1 class="text-4xl font-extrabold text-primary mb-2"><?php echo htmlspecialchars($quizzData['Name']); ?></h1>
                ...
                <input type="hidden" name="quizzId" value="<?= htmlspecialchars($quizz); ?>">
                    ...
                    <?php if (is_array($question)): ?>
                        <?php if ($question['Type'] == "libre"): ?>
                            <div class="question p-4 bg-gray-200 rounded-lg border border-gray-200">
                                <label for="answer-<?php echo $question['Position']; ?>" class="label-text font-semibold text-lg mb-2 block"><?php echo htmlspecialchars($question['Question']); ?></label>
                        ...
                        <?php if ($question['Type'] == "checkbox"): ?>
                            <div class="question p-4 bg-gray-200 rounded-lg border border-gray-200">
                                <label for="answer-<?php echo $question['Position']; ?>" class="label-text font-semibold text-lg mb-4 block"><?php echo htmlspecialchars($question['Question']); ?></label>
                                <div class="space-y-2">
                                    <?php foreach($reponses[$question['Position']] as $answer): ?>
                                        <div class="form-control flex-row items-center space-x-3">
                                            ...
                                            <label for="answer-<?php echo $question['Position']; ?>-<?php echo $answer['ID']; ?>" class="label cursor-pointer">
                                                <span class="label-text"><?php echo htmlspecialchars($answer['Text']); ?></span>
                                            </label>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                    ...
                    <?php if (is_array($question)): ?>
                        <?php if ($question['Type'] == 'libre'): ?>
                            <div class="question p-4 bg-gray-200 rounded-lg border border-gray-200">
                                ...
                                <label for="answer-<?php echo $question['Position']; ?>" class="label-text font-semibold text-lg mb-2 block"><?php echo htmlspecialchars($question['Question']); ?> <span class="text-accent font-normal text-sm">-- <?= $question['Points'] ?> Points</span></label>
                                <input type="text" name="answer[<?php echo $question['Position']; ?>]" id="answer-<?php echo $question['Position']; ?>" required class="input input-bordered w-full">
                            </div>
                        <?php endif; ?>
                        <?php if ($question['Type'] == 'checkbox'): ?>
                            <div class="question p-4 bg-gray-200 rounded-lg border border-gray-200">
                                <label for="answer-<?php echo $question['Position']; ?>" class="label-text font-semibold text-lg mb-4 block"><?php echo htmlspecialchars($question['Question']); ?> <span class="text-accent font-normal text-sm">-- <?= $question['Points'] ?> Points</span></label>
                                <div class="space-y-2">
                                    <?php foreach($reponses[$question['Position']] as $answer): ?>
                                        <div class="form-control flex-row items-center space-x-3">
                                            <input type="hidden" name="questionId[<?php echo $question['Position'] ?>]" value="<?php echo $question['ID']; ?>">
